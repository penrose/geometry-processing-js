...
const MeshSubset = require('../../core/mesh-subset.js')
...
class SimplicialComplexOperators {
        ...
        buildVertexEdgeAdjacencyMatrix(mesh) {
                let T = new LinearAlgebra.Triplet(mesh.edges.length, mesh.vertices.length);
                for (let e of mesh.edges) {
        ...
        buildEdgeFaceAdjacencyMatrix(mesh) {
                let T = new LinearAlgebra.Triplet(mesh.faces.length, mesh.edges.length);
                for (let f of mesh.faces) {
...
module.exports = { SimplicialComplexOperators }
		   // assignElementIndices, buildVertexEdgeAdjacencyMatrix, buildEdgeFaceAdjacencyMatrix, 
		   // star, closure, link, IsComplex, IsPureComplex, boundary }
